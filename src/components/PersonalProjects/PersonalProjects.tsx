...
export const PersonalProjects = forwardRef<HTMLElement>(function Work(_props, forwardedRef) {
  const { t } = useLanguage();
  ...
  return (
    ...
    >
      <div className={[shared.eyebrow, inView ? shared.inView : ''].filter(Boolean).join(' ')}>
      ...
      <div className={styles.list}>
        {ITEMS.map((item) => (
          <div
            key={item.num}
            className={[styles.item, item.delayClass, inView ? shared.inView : '']
              .filter(Boolean)
              .join(' ')}
          >
